...
Цель проверки — понять, какие структуры WB/Ozon встречаются в открытых примерах, сравнить их с текущим preview-адаптером и зафиксировать, что нужно для безопасной production-версии импорта.

Сырые публичные файлы не добавляются в репозиторий и не становятся fixtures. Если пример нужен для автотестов или демо, на его основе создаётся собственный synthetic production-like шаблон без чужих коммерческих данных.
...
- отдельная версия формата, например `wb:finance-report:api-csv:preview-2026-07` — добавлена;
- поддержка `supplier_oper_name` как основного типа операции — добавлена для товарных продаж и возвратов;
- сверка без `for_pay`, если в выгрузке его нет — добавлена через явное missing-поле `forPayReconciliation`;
- явная обработка операционных строк вроде возмещения издержек, а не только `Продажа`/`Возврат` — service-only строки без SKU пока не распределяются по товарам и дают предупреждение или ошибку;
- учёт `acquiring_fee`, `acceptance`, `payment_processing`, `cashback_amount` и других новых удержаний — добавлен для preview-расчёта;
- ошибка без вывода строк отчёта, если файл содержит контрагентов или другие чувствительные поля — сохраняется через публичные коды ошибок без печати исходных строк.
...
Первый шаг выполнен: добавлены negative-detection для WB XLSX-каталога, synthetic CSV fixture по структуре публичного WB CSV и preview-адаптер для товарных строк CSV. Следующий шаг — развивать собственные synthetic WB/Ozon-шаблоны по безопасно подтверждённой схеме и сохранять статус preview, пока production-формат не будет подтверждён анонимизированной выгрузкой или официальной спецификацией.
