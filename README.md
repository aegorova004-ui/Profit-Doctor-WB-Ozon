...
Auth email/OTP delivery пока выключен. Переменные `AUTH_EMAIL_PROVIDER`, `AUTH_EMAIL_FROM` и `AUTH_EMAIL_API_KEY` описаны в `.env.example`; shortlist провайдеров — в [`docs/auth-email-provider-options.md`](docs/auth-email-provider-options.md).
