...
На странице `/upload` первый preview-адаптер принимает XLSX с API-подобными полями финансового отчёта Wildberries. Он читает файл локально, переводит денежные значения в integer-копейки, объединяет продажи и возвраты по SKU и сверяет результат с `for_pay`. После импорта можно ввести себестоимость единицы, получить пересчёт до рекламы, раскрыть источники каждой суммы и увидеть убыточные SKU. Диагноз ранжирует проблемные товары, считает разрыв до безубыточности и показывает предельную себестоимость по известным расходам. Это ещё не поддержка фактической выгрузки из кабинета: она будет объявлена только после проверки актуального анонимизированного образца.
...
Финансовые правила описаны в [`docs/finance-formulas.md`](docs/finance-formulas.md), контракт импорта — в [`docs/report-formats.md`](docs/report-formats.md), текущий план — в [`docs/sprint-1-plan.md`](docs/sprint-1-plan.md), подготовка релиза — в [`docs/release-checklist.md`](docs/release-checklist.md), варианты российского эквайринга — в [`docs/payment-provider-options.md`](docs/payment-provider-options.md).
