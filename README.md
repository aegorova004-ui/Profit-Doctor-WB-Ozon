...
Проект находится на этапе `Sprint 1: Первый полезный импорт`. Preview-адаптер локально разбирает синтетический XLSX финансового отчёта WB, принимает себестоимость единицы по SKU и показывает источники сумм и убыточные товары без отправки файла на сервер. Текущий статус и ближайшие задачи описаны в [`PROJECT_STATE.md`](PROJECT_STATE.md), план этапов — в [`ROADMAP.md`](ROADMAP.md).
